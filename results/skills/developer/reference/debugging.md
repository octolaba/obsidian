...
| Settings changes vanish after reload | A `render` callback mutated settings — "Obsidian only saves automatically for `control` bindings" (docs: en/Plugins/User interface/Settings.md:740-741); or an overridden `setControlValue` "replaces the default write path, including the automatic `saveData()` call" (docs: en/Plugins/User interface/Settings.md:377) — declarative API, `@since 1.13.0`, stable at pin (docs: en/Plugins/User interface/Settings.md:8) | `await this.plugin.saveData(...)` in `render`; persist and return the promise in the override |
