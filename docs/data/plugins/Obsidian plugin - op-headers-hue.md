---
uid: ce71737a-dfe9-5c60-a544-389b91e279bc
xid:
  - op-headers-hue
aliases:
  - op-headers-hue
  - Headers Hue
  - markcrobbins2112/op-headers-hue
tags:
  - type/bookmark
  - bookmark/obsidian
  - obsidian/plugin
url: https://community.obsidian.md/plugins/op-headers-hue
alt:
  - https://github.com/markcrobbins2112/op-headers-hue
downloads:
updated at:
related to:
  - "[[GitHub - 1281091710]]"
remind me:
---

# Headers Hue

Headers Hue colours Markdown headers from H1 to H6 with a distinct hue per level and directional gradient borders. Header colours lighten on hover, and custom hue shifts are animated when a fold state is updated.

```cue
plugin: {
    id:     "op-headers-hue"
    name:   "Headers Hue"
    author: "markcrobbins"
    repo:   "markcrobbins2112/op-headers-hue"

    html_url:    "https://community.obsidian.md/plugins/op-headers-hue"
    github_url:  "https://github.com/markcrobbins2112/op-headers-hue"
    description: "Colors markdown headers from H1-H6, adds directional gradient borders, handles hover lightening, and plays custom hue shifts during fold state updates. - This plugin has not been manually reviewed by Obsidian staff."
    about:       "Color Markdown headers H1–H6 with directional gradient borders and distinct hues per level. Lighten header colors on hover and animate custom hue shifts when folding or updating header states."
}
```

[^template]: [[Obsidian plugin]]
